    ...
    listEl.innerHTML = sorted.map((record, index) => {
        ...
        return `
            <div class="sleep-record-item" style="${isLast ? 'border-left:3px solid var(--accent-color);' : ''}">
                ...
                <button class="sleep-record-delete-btn" onclick="event.stopPropagation(); deleteSleepRecord(${record.id})" title="删除此记录">
...
// 删除单条睡眠记录
window.deleteSleepRecord = async function(recordId) {
    if (!confirm('确定要删除这条睡眠记录吗？')) return;
    const history = await getSleepHistory();
    const newHistory = history.filter(r => r.id !== recordId);
    await saveSleepHistory(newHistory);
    renderSleepReport();
