...
window.exportByDateRange = function() {
    ...
    overlay.innerHTML = `
        <div style="background:var(--secondary-bg);border-radius:20px;padding:24px;width:90%;max-width:380px;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:modalContentSlideIn 0.3s ease forwards;">
            <div style="font-size:16px;font-weight:700;color:var(--text-primary);margin-bottom:6px;display:flex;align-items:center;gap:8px;">
                <i class="fas fa-calendar-alt" style="color:var(--accent-color);"></i>按日期导出
            </div>
            <div style="font-size:12px;color:var(--text-secondary);margin-bottom:16px;">选择日期范围，导出该时间段内的全部消息</div>
            <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:18px;">
                <div>
                    <label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:4px;">开始日期</label>
                    <input type="date" id="export-date-from" style="width:100%;padding:10px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);color:var(--text-primary);font-size:14px;font-family:var(--font-family);">
                </div>
                <div>
                    <label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:4px;">结束日期</label>
                    <input type="date" id="export-date-to" style="width:100%;padding:10px 12px;border:1px solid var(--border-color);border-radius:10px;background:var(--primary-bg);color:var(--text-primary);font-size:14px;font-family:var(--font-family);">
                </div>
            </div>
            <div style="display:flex;gap:10px;">
                <button id="export-date-cancel" style="flex:1;padding:11px;border:1px solid var(--border-color);border-radius:12px;background:none;color:var(--text-secondary);font-size:13px;cursor:pointer;font-family:var(--font-family);">取消</button>
                <button id="export-date-confirm" style="flex:2;padding:11px;border:none;border-radius:12px;background:var(--accent-color);color:#fff;font-size:13px;font-weight:600;cursor:pointer;font-family:var(--font-family);">导出</button>
            </div>
        </div>
    `;
    ...
    document.getElementById('export-date-cancel').onclick = close;

    document.getElementById('export-date-confirm').onclick = function() {
        var fromVal = document.getElementById('export-date-from').value;
        var toVal = document.getElementById('export-date-to').value;

        if (!fromVal && !toVal) {
            if (typeof showNotification === 'function') showNotification('请至少选择一个日期', 'warning');
            return;
        }
        ...
        if (filtered.length === 0) {
            if (typeof showNotification === 'function') showNotification('该日期范围内没有消息', 'warning');
            return;
        }
        ...
        filtered.forEach(function(msg) {
            var sender = msg.sender === 'user' ? myName : partnerName;
...
if (!content && msg.image) {
    try {
        var savedNotes = localStorage.getItem('stickerNotes');
        var notes = savedNotes ? JSON.parse(savedNotes) : {};
        content = notes[msg.image] || msg.note || '[图片]';
    } catch(e) {
        content = msg.note || '[图片]';
    }
}
textContent += '[' + time + '] ' + sender + '：\n' + content + '\n\n';
        ...
        var dateRange = (fromVal || '最早') + ' 至 ' + (toVal || '最晚');
        showExportPreviewWindow(textContent, dateRange);
    };
};

function showExportPreviewWindow(text, description) {
    var old = document.getElementById('export-preview-overlay');
    if (old) old.remove();
    ...
    overlay.innerHTML =
        '<div style="background:var(--secondary-bg);border-radius:20px;padding:20px;width:90%;max-width:550px;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.4);">' +
            '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">' +
                '<div style="font-size:16px;font-weight:700;color:var(--text-primary);">' +
                    '<i class="fas fa-file-alt" style="color:var(--accent-color);margin-right:6px;"></i>导出预览' +
                    '<span style="font-size:12px;color:var(--text-secondary);font-weight:400;margin-left:8px;">' + description + '</span>' +
                '</div>' +
                '<button onclick="document.getElementById(\'export-preview-overlay\').remove()" style="' +
                    'width:28px;height:28px;border-radius:50%;border:none;' +
                    'background:var(--primary-bg);color:var(--text-secondary);' +
                    'cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:14px;' +
                '">✕</button>' +
            '</div>' +
            '<textarea id="export-preview-text" readonly style="' +
                ...
                'font-family:var(--font-family); line-height:1.8;' +
                'resize:none; outline:none; white-space:pre-wrap;' +
            '">' + text.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</textarea>' +
            '<div style="display:flex;gap:10px;margin-top:14px;">' +
                '<button id="export-preview-copy" style="' +
                    'flex:1; padding:12px; border:1px solid var(--border-color);' +
                    'border-radius:12px; background:var(--primary-bg);' +
                    'color:var(--text-primary); cursor:pointer; font-size:13px;' +
                    'font-family:var(--font-family); display:flex; align-items:center;' +
                    'justify-content:center; gap:6px;' +
                '"><i class="fas fa-copy"></i> 复制全文</button>' +
                '<button id="export-preview-download" style="' +
                    'flex:1; padding:12px; border:none; border-radius:12px;' +
                    'background:var(--accent-color); color:#fff; cursor:pointer;' +
                    'font-size:13px; font-family:var(--font-family); font-weight:600;' +
                    'display:flex; align-items:center; justify-content:center; gap:6px;' +
                '"><i class="fas fa-download"></i> 下载 TXT</button>' +
            '</div>' +
        '</div>';
    ...
    var copyBtn = overlay.querySelector('#export-preview-copy');
    if (copyBtn) {
        copyBtn.addEventListener('click', function() {
            var textarea = overlay.querySelector('#export-preview-text');
            try {
                navigator.clipboard.writeText(textarea.value);
                if (typeof showNotification === 'function') showNotification('已复制到剪贴板', 'success', 2000);
            } catch (e) {
                textarea.select();
                if (typeof showNotification === 'function') showNotification('请手动复制（Ctrl+C / 长按全选）', 'info', 2000);
            }
        });
    }

    var downloadBtn = overlay.querySelector('#export-preview-download');
    if (downloadBtn) {
...
var batchExportMode = false;
var batchExportSelected = new Set();

window.enterBatchExportMode = function() {
    batchExportMode = true;
    batchExportSelected.clear();
    document.body.classList.add('batch-export-mode');
    showBatchExportToolbar();
    // 重新渲染，让所有消息获得点击事件和视觉反馈
    if (typeof renderMessages === 'function') renderMessages(true);
    if (typeof showNotification === 'function') showNotification('点击聊天界面中的消息即可选中', 'info', 3000);
};

window.exitBatchExportMode = function() {
    batchExportMode = false;
    batchExportSelected.clear();
    document.body.classList.remove('batch-export-mode');
    hideBatchExportToolbar();
    if (typeof renderMessages === 'function') renderMessages(true);
};

function showBatchExportToolbar() {
    if (document.getElementById('batch-export-toolbar')) return;
    var toolbar = document.createElement('div');
    toolbar.id = 'batch-export-toolbar';
    toolbar.style.cssText = 'position:fixed;bottom:30px;left:50%;transform:translateX(-50%);background:var(--secondary-bg);border:1px solid var(--border-color);border-radius:20px;padding:8px 14px;z-index:10000;display:flex;gap:10px;align-items:center;box-shadow:0 4px 20px rgba(0,0,0,0.15);font-family:var(--font-family);';
    toolbar.innerHTML = '<span id="batch-export-count" style="font-size:13px;color:var(--text-primary);">已选 0 条</span>' +
        '<button onclick="exitBatchExportMode()" style="padding:6px 12px;border:1px solid var(--border-color);border-radius:8px;background:var(--primary-bg);color:var(--text-secondary);cursor:pointer;font-size:12px;font-family:var(--font-family);">取消</button>' +
        '<button onclick="confirmBatchExport()" style="padding:6px 14px;border:none;border-radius:8px;background:var(--accent-color);color:#fff;cursor:pointer;font-size:12px;font-family:var(--font-family);font-weight:600;">确认导出</button>';
    document.body.appendChild(toolbar);
}

function hideBatchExportToolbar() {
    var toolbar = document.getElementById('batch-export-toolbar');
    if (toolbar) toolbar.remove();
}

// 核心：通过事件委托监听聊天界面的点击，实现消息选中/取消
document.addEventListener('click', function(e) {
    if (!batchExportMode) return;
    var wrapper = e.target.closest('.message-wrapper');
    if (!wrapper) return;
    // 防止点击到按钮等元素时触发
    if (e.target.closest('.meta-action-btn') || e.target.closest('.delete-btn') || e.target.closest('.favorite-action-btn') || e.target.closest('.reply-btn')) return;

    var msgId = parseInt(wrapper.dataset.msgId || wrapper.dataset.id);
    if (!msgId) return;

    if (batchExportSelected.has(msgId)) {
        batchExportSelected.delete(msgId);
        wrapper.classList.remove('batch-selected');
    } else {
        batchExportSelected.add(msgId);
        wrapper.classList.add('batch-selected');
    }

    var countEl = document.getElementById('batch-export-count');
    if (countEl) countEl.textContent = '已选 ' + batchExportSelected.size + ' 条';
});

// 当消息列表重新渲染后，恢复已选中的高亮状态
var origRenderMessages = window.renderMessages;
if (origRenderMessages) {
    window.renderMessages = function(preserveScroll) {
        origRenderMessages(preserveScroll);
        if (!batchExportMode) return;
        // 给所有已选中的消息加上高亮
        var container = document.getElementById('chat-container');
        if (!container) return;
        var wrappers = container.querySelectorAll('.message-wrapper');
        wrappers.forEach(function(w) {
            var msgId = parseInt(w.dataset.msgId || w.dataset.id);
            if (batchExportSelected.has(msgId)) {
                w.classList.add('batch-selected');
            }
        });
    };
}

window.confirmBatchExport = function() {
    if (batchExportSelected.size === 0) {
        if (typeof showNotification === 'function') showNotification('请先在聊天界面点击选择消息', 'warning');
        return;
    }

    var selectedMessages = (typeof messages !== 'undefined' ? messages : [])
        .filter(function(m) { return batchExportSelected.has(m.id) && m.type !== 'system'; });
...
textContent += '[' + time + '] ' + sender + '：\n' + content + '\n\n';
    ...
    var description = '已选 ' + selectedMessages.length + ' 条消息';
    exitBatchExportMode();
    showExportPreviewWindow(textContent, description);
};

// 绑定按钮事件
document.addEventListener('click', function(e) {
    if (e.target.id === 'export-by-date-btn' || e.target.closest('#export-by-date-btn')) {
        window.exportByDateRange();
    }
    if (e.target.id === 'batch-export-btn' || e.target.closest('#batch-export-btn')) {
        window.enterBatchExportMode();
    }
});