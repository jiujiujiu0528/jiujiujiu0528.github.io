...
// 贴纸注释存储
if (typeof stickerNotes === 'undefined') var stickerNotes = {};

(function() {
    try {
        var saved = localStorage.getItem('stickerNotes');
        if (saved) stickerNotes = JSON.parse(saved);
    } catch(e) { stickerNotes = {}; }
})();

function saveStickerNotes() {
    try {
        localStorage.setItem('stickerNotes', JSON.stringify(stickerNotes));
    } catch(e) {}
}
    ...
    itemsToRender.forEach((item, index) => {
        ...
        div.innerHTML = `
            <div style="position:relative;width:100%;aspect-ratio:1/1;overflow:hidden;border-radius:8px;">
                <img src="${item}" loading="lazy" style="width:100%;height:100%;object-fit:cover;">
                <div class="sticker-batch-check" style="position:absolute;top:4px;left:4px;">✓</div>
                <div class="sticker-delete-btn" style="position:absolute;top:4px;right:4px;"><i class="fas fa-times"></i></div>
            </div>
            <div class="sticker-note-display" style="
                width:100%;
                padding:2px 4px;
                font-size:10px;
                color:${note ? 'var(--accent-color)' : 'var(--text-secondary)'};
                text-align:center;
                white-space:nowrap;
                overflow:hidden;
                text-overflow:ellipsis;
                opacity:${note ? '0.85' : '0.5'};
                cursor:pointer;
        ...
        // 点击注释区域：弹出编辑弹窗
        div.querySelector('.sticker-note-display').addEventListener('click', function(e) {
            e.stopPropagation();
            openStickerNoteEditor(item, index);
        });
...
// 贴纸注释编辑弹窗
function openStickerNoteEditor(stickerSrc, stickerIndex) {
    var currentNote = stickerNotes[stickerSrc] || '';
    
    var old = document.getElementById('sticker-note-editor-overlay');
    if (old) old.remove();
    
    var overlay = document.createElement('div');
    overlay.id = 'sticker-note-editor-overlay';
    ...
    overlay.innerHTML = `
        <div style="background:var(--secondary-bg);border-radius:20px;padding:20px;width:88%;max-width:340px;box-shadow:0 20px 60px rgba(0,0,0,0.4);animation:modalContentSlideIn 0.3s ease forwards;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
                <img src="${stickerSrc}" style="width:44px;height:44px;border-radius:10px;object-fit:cover;border:1px solid var(--border-color);flex-shrink:0;">
                <div>
                    <div style="font-size:14px;font-weight:700;color:var(--text-primary);">添加注释</div>
                    <div style="font-size:11px;color:var(--text-secondary);">为这张表情包添加描述文字</div>
                </div>
            </div>
            <input type="text" id="sticker-note-input" value="${currentNote.replace(/"/g, '&quot;')}" placeholder="例：开心的猫猫 / 生气 / 晚安" maxlength="30" style="
                ...
                font-size:14px;font-family:var(--font-family);outline:none;box-sizing:border-box;
                transition:border-color 0.2s;
            ">
            <div style="font-size:11px;color:var(--text-secondary);margin-top:8px;opacity:0.6;">
                导出聊天记录时，会显示此注释代替空白图片
            </div>
            <div style="display:flex;gap:10px;margin-top:14px;">
                <button id="sticker-note-cancel" style="flex:1;padding:11px;border:1px solid var(--border-color);border-radius:12px;background:none;color:var(--text-secondary);font-size:13px;cursor:pointer;font-family:var(--font-family);">取消</button>
                <button id="sticker-note-save" style="flex:2;padding:11px;border:none;border-radius:12px;background:var(--accent-color);color:#fff;font-size:13px;font-weight:600;cursor:pointer;font-family:var(--font-family);">保存</button>
            </div>
        </div>
    `;
    ...
    var input = overlay.querySelector('#sticker-note-input');
    setTimeout(function() { input.focus(); input.select(); }, 300);
    
    function close() { overlay.remove(); }
    
    overlay.addEventListener('click', function(e) { if (e.target === overlay) close(); });
    overlay.querySelector('#sticker-note-cancel').onclick = close;
    
    overlay.querySelector('#sticker-note-save').onclick = function() {
        var val = input.value.trim();
        if (val) {
            stickerNotes[stickerSrc] = val;
        } else {
            delete stickerNotes[stickerSrc];
        }
        saveStickerNotes();
        close();
        renderReplyLibrary();
        if (typeof showNotification === 'function') {
            showNotification(val ? '注释已保存 ✓' : '注释已清除', 'success', 1500);
        }
    };
    
    input.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            overlay.querySelector('#sticker-note-save').click();
        }
    });
}
